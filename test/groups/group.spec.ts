...
test.group('Group', (group) => {
  ...
  test('it should return all groups by user id and name', async (assert) => {
    ...
    await supertest(BASE_URL)
      .post('/groups')
      .set('Authorization', `Bearer ${token}`)
      .send({ ...groupPayload, name: 'dois', description: '123' })
      .expect(201)

    const group = response.body.group

    const { body } = await supertest(BASE_URL)
      .get(`/groups?user=${user.id}&term=es`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200)
  ...
  test.only('it should return all groups by user id and description', async (assert) => {
    const groupPayload = {
      name: '123',
      description: 'test',
      schedule: 'test',
